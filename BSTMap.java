...
public class BSTMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    protected Node<K, V> createBST(Pair<K, V>[] pairs, int low, int high) {
        ...
        int mid = (low + high) / 2;
        Pair<K, V> middlePair = pairs[mid];

        Node<K, V> n = new Node<>(null, null);
        n.key = middlePair.key;
        n.value = middlePair.value;


        n.left = createBST(pairs, low, mid - 1);
        if(n.left!=null){
            n.left.parent = n;
        }
        n.right = createBST(pairs, mid + 1, high);
        if(n.right!=null){
            n.right.parent = n;
        }
        size++;
    ...
    private int preorder(Node<K, V> n, StringBuilder builder,
        ...
        if(n != null){
            ...
            int left = preorder(n.left, builder, nodesVisited);
            int right = preorder(n.right, builder, left);
            return right;
        }
        return nodesVisited;
    }
    ...
    private int inorder(Node<K, V> n, StringBuilder builder,
        ...
        if(n != null){
            ...
            int left = inorder(n.left, builder, nodesVisited);
            nodesVisited = left;
            if(nodesVisited > 0) {
                builder.append(", ");
            }
            ...
            nodesVisited++;
            //when nodes visited isn't zero

            int right = inorder(n.right, builder, nodesVisited);
    ...
    private Node<K, V> iterativeSearch(K key) {
        ...
        while (n !=null){
            ...
            if (cmp == 0) {
                ...
                n = n.left; // Key may be in the left subtree
            } else {
                n = n.right; // Key may be in the right subtree
            }
        }
    ...
    public void transplant(Node<K, V> u, Node<K, V> v){
        ...
        else if(u==u.parent.left){ //if its left subtree
            u.parent.left = v;
        }
